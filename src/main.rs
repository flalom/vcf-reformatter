...
use crate::reformat_vcf::{reformat_vcf_data_with_header, reformat_vcf_data_with_header_parallel, write_reformatted_vcf, TranscriptHandling};
use crate::extract_sample_info::*;
...
fn main() {
    ...
    println!("✅ File read completed in {:.2?}", read_time);
    ...
    println!("✅ Header file written in {:.2?}", header_write_time);
    println!();
    ...
    let result = if use_parallel {
        if cli.verbose {
            println!("   🚀 Using parallel processing with {} threads...", thread_count);
        }
    ...
    match result {
        Ok((headers, records)) => {
            ...
            println!("✅ Data processing completed in {:.2?}", process_time);
            println!("   📈 Processing rate: {:.0} variants/sec", variants_per_sec);
            println!("   📊 Output records: {} ({}x expansion)", records.len(),
                     format!("{:.2}", records.len() as f64 / data.2.len() as f64));
            if use_parallel {
                println!("   🚀 Parallel efficiency: {:.1}x speedup potential", thread_count as f64);
            }
            println!();

            // Write reformatted file
            println!("💾 Writing reformatted file{}...", if cli.compress { " (compressed)" } else { "" });
            let write_start = Instant::now();
            if let Err(e) = write_reformatted_vcf(&reformatted_file, &headers, &records, cli.compress) {
            ...
            println!("✅ Reformatted file written in {:.2?}{}", write_time, if cli.compress { " (compressed)" } else { "" });
            println!();

            // Print final summary
            print_final_summary(&cli, &header_file, &reformatted_file, &data, &records,
                                read_time, process_time, write_time, header_write_time,
                                total_time, variants_per_sec, thread_count, use_parallel);
        }
        Err(e) => {
...
fn print_startup_info(cli: &Cli, thread_count: usize, use_parallel: bool, transcript_handling: TranscriptHandling) {
    println!("🧬 VCF Reformatter Starting...");
    println!("📁 Input file: {}", cli.input_file);
    println!("🔧 Transcript handling: {}", match transcript_handling {
    ...
    println!("⚡ Processing mode: {} (using {} thread{})",
...
fn generate_output_filenames(cli: &Cli) -> (String, String) {
    ...
    let output_dir = cli.output_dir.as_ref().map(|s| s.as_str()).unwrap_or(".");

    let header_file = format!("{}/{}_header.txt", output_dir, base_name);
    let reformatted_file = if cli.compress {
        format!("{}/{}_reformatted.tsv.gz", output_dir, base_name)
    } else {
        format!("{}/{}_reformatted.tsv", output_dir, base_name)
    };
...
fn print_final_summary(
    ...
    println!("     • Reformatted: {}{}", reformatted_file,
             if cli.compress { " (gzip compressed)" } else { "" });
    println!();

    if cli.verbose {
        println!("⏱️  Performance Statistics:");
        println!("   📖 File reading:     {:.2?} ({:.1}% of total)",
                 read_time, (read_time.as_secs_f64() / total_time.as_secs_f64()) * 100.0);
        println!("   🔄 Data processing:  {:.2?} ({:.1}% of total)",
                 process_time, (process_time.as_secs_f64() / total_time.as_secs_f64()) * 100.0);
        println!("   💾 File writing:     {:.2?} ({:.1}% of total)",
                 write_time, (write_time.as_secs_f64() / total_time.as_secs_f64()) * 100.0);
        println!("   📝 Header writing:   {:.2?} ({:.1}% of total)",
                 header_write_time, (header_write_time.as_secs_f64() / total_time.as_secs_f64()) * 100.0);
        println!("   🕐 Total time:       {:.2?}", total_time);
        println!();
        println!("📊 Processing Rates:");
        println!("   🧬 Overall rate:     {:.0} variants/sec",
                 data.2.len() as f64 / total_time.as_secs_f64());
        println!("   📝 Output rate:      {:.0} records/sec",
                 records.len() as f64 / total_time.as_secs_f64());
        println!("   💽 Processing only:  {:.0} variants/sec", variants_per_sec);

        // Memory and efficiency stats
        println!();
        println!("📈 Efficiency Metrics:");
        println!("   📊 Expansion ratio:  {:.2}x (output/input records)",
                 records.len() as f64 / data.2.len() as f64);
        println!("   🏃 Speed vs baseline: {}x faster than 1,000 variants/sec",
                 (variants_per_sec / 1000.0).round() as u32);

        // Size estimation
        let estimated_input_size = estimate_file_size(&data.2);
        println!("   💾 Estimated input:  {}", format_file_size(estimated_input_size));
        ...
        print_performance_analysis(read_time, process_time, write_time, total_time,
                                   variants_per_sec, thread_count, use_parallel, data.2.len());
    } else {
...
fn get_base_filename(file_path: &str) -> String {
    ...
    let filename = path.file_name().unwrap_or_default().to_str().unwrap_or("output");

    // Remove common VCF extensions
    let base = filename
        .strip_suffix(".vcf.gz").unwrap_or(filename)
        .strip_suffix(".vcf").unwrap_or(filename);
